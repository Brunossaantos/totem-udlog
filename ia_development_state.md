...
| Migração da integração de CNH/CRLV do Serpro (VIO Decode direto) para a API contratada `https://vio.api.br` (validação assíncrona + comparação OCR + cache local) | `app/Rn/VioDecodeClient.php` (mantido intocado), `app/Rn/VioApiBrClient.php` (novo), `app/Controller/DocumentoController.php`, `app/Rn/DocumentoRn.php`, `app/Dao/AtendimentoDao.php`, `app/Dao/VioApiBrCacheDao.php` (novo), `sql/migrations/015_vio_api_br_estados_e_id_externo.sql`/`016_vio_api_br_cache.sql` (novas) | `/00-planejamento` concluído em 2026-09-25. **`/01-implementacao` CONCLUÍDA em 2026-09-25** (backend completo) — ver `docs/handoffs/2026-09-25-migracao-vio-api-br-com-cache.md`, seção "`/01-implementacao` — resultado (2026-09-25)", para a lista completa de arquivos/decisões. Resumo: novo `App\Rn\VioApiBrClient` (2 métodos, sem polling interno, HTTPS/fail-closed, transporte HTTP injetável para teste); nova máquina de estados assíncrona (`PENDENTE/ENVIANDO/PROCESSANDO_LEITURA/PROCESSANDO_COMPARACAO/CONCLUIDO/ERRO/INDETERMINADO`) via migration 015 (aditiva, `VioDecodeClient`/cache antigo intocados); novo cache seguro `VIO_CACHE` (`tb_vio_api_cache_cnh/crlv`, migration 016, fingerprint HMAC versionado, AES-256-GCM via `Util\CriptografiaHelper`, TTL 7 dias); CAS de envio + ID externo persistido antes do polling + dupla checagem (`tentativa_id` + status do atendimento) na gravação do resultado; PDF de 2 páginas da CNH gerado reaproveitando `Util\AnexoPdfHelper` já existente (sem lib nova); critério de aprovação automática completo (reliable/mismatched/campos críticos/páginas) implementado em `DocumentoRn`; `.env.example` com bloco `VIO_API_BR_*` (5 variáveis novas). Zero chamada real/paga, zero uso de credencial, zero migration aplicada em banco real, zero commit/push nesta rodada (fica para depois de `/02-testes`/`/03-revisao`). Trello não usado. **PENDENTE**: `/02-testes` (incluindo aplicar as migrations 015/016 em banco descartável) e `/03-revisao`; frontend (`frontend-especialista`, rodada separada) ainda precisa inverter ordem do CRLV e construir a tela de estados intermediários; contrato real da `vio.api.br` continua tratado como placeholder fornecido pelo usuário (14 perguntas ao fornecedor do `/00-planejamento` seguem sem resposta oficial, incluindo o formato exato de "saldo insuficiente"); nova credencial rotacionada da vio.api.br ainda não existe/não foi usada. **RODADA CORRETIVA `/01-implementacao` em 2026-09-26** (backend, ver `docs/handoffs/2026-09-25-migracao-vio-api-br-com-cache.md`, seção "Rodada corretiva (2026-09-26)"): (1) confirmado por leitura de código que o backend JÁ garante 1 única tentativa de validação externa por documento mesmo para o Recebimento (tipo `cnh` só existe como unidade indivisível no fluxo de processamento — nunca `cnh_frente`/`cnh_verso` isolados — e a montagem do PDF falha com segurança se um dos 2 lados ainda não estiver em disco, sem nunca chegar a gerar POST); adicionado gate explícito ANTES do CAS de envio (`DocumentoController::arquivosCnhAmbosLadosPresentes()`) só para evitar desperdiçar um ciclo de CAS/ERRO nesse cenário previsível, sem alterar a máquina de etapas; (2) origem de auditoria separada — `VIO_API_BR` é agora um valor de `ENUM` próprio (migration 015 editada diretamente, nunca aplicada em banco real) para a validação real concluída pela vio.api.br, `VIO_VALIDADO` volta a significar exclusivamente o histórico do Serpro/`VioDecodeClient` (nunca mais gravado por código novo), `VIO_CACHE`/`MANUAL` inalterados; allowlists de aprovação/reaproveitamento de `rntc`/`tipo_veiculo` e a preservação de snapshot (`AtendimentoDao::atualizarValidacaoCnh/Crlv`) atualizadas para incluir `VIO_API_BR`; `VioApiBrCacheDao` ganhou filtro explícito `origem_original = 'VIO_API_BR'` (defesa em profundidade); (3) reconciliação de kiosk — novo `AtendimentoDao::reconciliarProcessamentoVioApiBrAbandonado()`, chamado a cada novo aceite LGPD/criação de atendimento, transiciona para `INDETERMINADO` (nunca `PENDENTE`) qualquer tentativa vio.api.br ainda `ENVIANDO`/`PROCESSANDO_LEITURA`/`PROCESSANDO_COMPARACAO` de um atendimento ANTERIOR abandonado do mesmo totem, sem nenhuma chamada de rede e sem tocar no atendimento novo/já finalizado; (4) as 4 suítes de teste manual antigas (`teste_integridade_conclusao_atendimento.php`, `teste_lock_obter_lock_documento.php`, `teste_sanitizacao_logs_documento_nota.php`, `teste_status_processamento.php`, mais os helpers `_caso_iniciar_processamento_vio_indisponivel.php`/`_caso_iniciar_processamento_falha_durante_validacao.php`) foram atualizadas para exercitar `VioApiBrClient`/os novos estados assíncronos em vez do `VioDecodeClient`/estado `PROCESSANDO` único antigo, preservando a mesma cobertura (sanitização de log, lock/mutex, CAS, estados terminais, ausência de retry automático, resultado tardio, fallback manual). **PENDENTE**: aplicar/validar migrations 015/016 em banco descartável e rodar as 4 suítes reescritas (`/02-testes`, próxima etapa, fora desta rodada). **RODADA `/02-testes` (QA corretiva, 2026-09-26)** — ver `docs/handoffs/2026-09-25-migracao-vio-api-br-com-cache.md`, seção "`/02-testes` — rodada de QA corretiva (2026-09-26)": `tests/manual/_qa_db_bootstrap.php` renomeado para `tests/manual/qa_db_bootstrap.php` (versionável, confirmado por `git check-ignore -v` que não é mais ignorado) e as 2 suítes que o usam atualizadas; árvore hermética (`git write-tree`/`git archive`) confirmou as 3 suítes novas (`teste_vio_api_br_client.php` 77/77, `teste_vio_api_br_cas_e_cache.php` 46/46, `teste_vio_api_br_migrations.php` 29/29) reproduzíveis em clone limpo, zero dependência de arquivo `_*.php` não versionado. Resíduo sintético autorizado (`TESTE_VIO`/`TESTE_REBAIXA`/`TESTE_REC_DOC` em `tb_totem`) removido de `udlog_totem` via protocolo de transação com contagem antes/depois (12→9 registros, exatamente 3 excluídos, sem dependências em nenhuma outra tabela); os 3 scripts que geravam esse resíduo (`teste_vio_decode.php`, `teste_rebaixamento_manual.php`, `teste_fluxo_recebimento_documentos.php`) corrigidos para sufixo aleatório no `codigo` + teardown via `register_shutdown_function`. **ACHADO BLOQUEANTE NOVO**: as migrations 015/016 nunca foram aplicadas ao banco de dev real `udlog_totem` (confirmado por `SHOW COLUMNS`/`SHOW TABLES` — colunas `*_vio_api_id` e tabelas `tb_vio_api_cache_*` ausentes) — isso faz com que as 4 suítes reescritas na rodada corretiva anterior, ao serem executadas de verdade pela primeira vez, falhem parcial ou totalmente ao exercitar `AtendimentoDao::iniciarEnvioVioApiBr()`/caminhos reais de `DocumentoController::iniciarProcessamento()` contra o schema desatualizado: `teste_integridade_conclusao_atendimento.php` 37/43 (Itens 9a/9b), `teste_lock_obter_lock_documento.php` 15/25, `teste_sanitizacao_logs_documento_nota.php` 27/29 (Ponto 1); `teste_status_processamento.php` nem chegou a rodar (residual pré-existente `codigo='TESTE_STATUS_PROC'`/`id_totem=2053`/`id_atendimento=9964`, criado em 2026-09-25 por execução anterior sem teardown seguro, causando `Duplicate entry` — NÃO removido nesta rodada por não constar na lista de resíduos autorizados da Tarefa 4). Decisão pendente do usuário/orquestrador: autorizar aplicar migration 015 (no mínimo) em `udlog_totem`, e autorizar a remoção do resíduo `TESTE_STATUS_PROC`. As 3 suítes novas (auto-contidas em banco `qa_` descartável) não são afetadas por este achado. **RODADA CORRETIVA `/02-testes` em 2026-09-26 (resolvida sem tocar `udlog_totem`)** — ver `docs/handoffs/2026-09-25-migracao-vio-api-br-com-cache.md`, seção "`/02-testes` — rodada corretiva de banco `qa_` próprio para as 4 suítes reescritas (2026-09-26)": as 4 suítes reescritas passam a rodar contra um banco `qa_`-prefixado descartável PRÓPRIO (mesmo `tests/manual/qa_db_bootstrap.php`, com `qaDbCorrigirEnumsMigration015()`), em vez de `udlog_totem` — achado técnico registrado (Dotenv imutável nunca promove sozinho um valor só-`getenv()` para `$_ENV` em subprocesso, exigindo ponte explícita `qaDbTrechoPonteEnvSubprocesso()`, novo helper compartilhado); `teste_integridade_conclusao_atendimento.php` também teve 6 subprocessos `_caso_*.php` nunca versionados inlinados (mesmo padrão já usado pelas outras 2 suítes). Resultado: as 7 suítes desta demanda somam 262/262 (77+46+29+13+43+25+29), árvore hermética reconfirmada para as 7 (não só as 3 de antes), zero banco `qa_` remanescente, `git diff --check` limpo, regressão ampla (33 suítes adicionais) 100% aprovada sem regressão. `TESTE_STATUS_PROC` deixa de ser bloqueio para qualquer suíte (nenhuma toca mais `udlog_totem`) — permanece como observação/resíduo órfão pré-existente, remoção ainda não autorizada. **PENDENTE** (fora do escopo de `/02-testes`, decisão de produto/infra): aplicar migrations 015/016 em `udlog_totem` real, e autorizar remoção do resíduo `TESTE_STATUS_PROC`. Regressão ampla (Talent 9 suítes, LGPD parcial, JPEG seguro, rate limit, concorrência, expedição, e2e recebimento/expedição, IDOR) 100% aprovada após atualização de 3 arquivos para a etapa `rec_cnh` unificada (`teste_e2e_recebimento_expedicao_mock.php`, `teste_idor_salvar_etapa_manual.php`, `teste_salvar_etapa_cliente_manual.php`) e reescrita completa de `teste_fluxo_recebimento_documentos.php` (agora cobre o gate `upload_cnh` bloqueando com só um lado da CNH presente). 2 achados pré-existentes, fora do escopo desta demanda, NÃO corrigidos: `teste_consulta_ordem_coleta.php` (11/17, divergência de fixture de dados no banco externo `udlogo59_db_gestao_coletas`, já registrado linha acima em 2026-09-17) e `teste_lgpd_aceite_backend_seguranca.php` (falha fatal, depende de `tests/manual/_fixtures_lgpd.php`, arquivo nunca versionado/nunca existiu no worktree — mesma classe de achado de reprodutibilidade da demanda `sanitizacao-excecoes-lock-documentos`, mas em teste de outra demanda/LGPD). **`/02-testes` INDEPENDENTE concluído em 2026-09-26 (veredito APROVADO)** — ver `docs/handoffs/2026-09-25-migracao-vio-api-br-com-cache.md`, seção "`/02-testes` independente — veredito final (2026-09-26)": 4 revisores independentes em instâncias novas, sem participação na implementação (frontend/UX 8/8, security-especialista 6/6 incl. 2 provas negativas, backend-especialista-revisor 6/6 provas negativas controladas em cópia isolada com worktree principal confirmado intocado hash-a-hash, qa-testes 9/9 incl. execução real das 7 suítes em banco `qa_` próprio = 262/262 e regressão ampla 100%), todos CONFIRMADOS. Zero chamada real/paga, zero credencial real, zero migration em banco real, zero Trello, zero alteração de código, zero commit/push nesta etapa. 3 pendências não-bloqueantes registradas para `/03-revisao`: (1) decisão do usuário sobre aplicar migrations 015/016 em `udlog_totem`; (2) decisão do usuário sobre remover o resíduo `TESTE_STATUS_PROC` em `udlog_totem`; (3) os 2 achados pré-existentes de outras demandas acima, reconfirmados sem correção. **CONVERGÊNCIA DE AMBIENTE DE DESENVOLVIMENTO em 2026-09-26 (migrations 015/016 aplicadas em `udlog_totem`)** — ver `docs/handoffs/2026-09-25-migracao-vio-api-br-com-cache.md`, seção "Convergência do ambiente de desenvolvimento — migrations 015/016 aplicadas em `udlog_totem` (2026-09-26)": autorização explícita do usuário, escopo EXCLUSIVO do banco de DEV LOCAL `udlog_totem` (nunca Hostgator/produção). Preflight confirmou `SELECT DATABASE()='udlog_totem'`, ausência de divergência estrutural (tabelas/estruturas esperadas do projeto presentes e consistentes com o já documentado), backup real via `mysqldump --routines --triggers --single-transaction` gerado fora do repositório antes de qualquer escrita. Migrations 015 e 016 aplicadas nesta ordem e REAPLICADAS uma 2ª vez (idempotência comprovada — `SHOW CREATE TABLE` idêntico entre a 1ª e a 2ª aplicação, zero duplicação de coluna/índice/tabela, zero erro). Contagens de `tb_totem`/`tb_atendimento`/`tb_cliente` idênticas antes e depois (9/127/38 — nenhuma linha adicionada/removida/alterada, migrations puramente aditivas). `tb_vio_api_cache_cnh`/`tb_vio_api_cache_crlv` criadas conforme especificação exata da migration 016 (0 linhas). Colunas novas de `tb_atendimento` (`cnh`/`crlv` × `vio_api_id`/`vio_api_enviado_em`/`vio_api_fingerprint`/`vio_api_fingerprint_versao`) e novos valores de `ENUM` (`ENVIANDO`/`PROCESSANDO_LEITURA`/`PROCESSANDO_COMPARACAO`/`INDETERMINADO` em `*_status_processamento`; `VIO_CACHE`/`VIO_API_BR` em `*_origem_validacao`) confirmados presentes e exatos. Zero `DROP`/`TRUNCATE`, zero chamada de rede real, zero uso de credencial (nem exibida nem logada). **PENDÊNCIA RESOLVIDA**: "migrations 015/016 nunca aplicadas em `udlog_totem`" não bloqueia mais nada — as 4 suítes reescritas continuam rodando contra banco `qa_` próprio (decisão já tomada na rodada corretiva de `/02-testes`, não revertida), então não dependiam desta aplicação para passar; nenhuma suíte desta demanda depende especificamente de `udlog_totem` já migrado. **RESÍDUO `TESTE_STATUS_PROC` REMOVIDO em 2026-09-26** (autorização explícita do usuário, escopo EXCLUSIVO deste marcador em `udlog_totem`, banco de DEV LOCAL) — ver `docs/handoffs/2026-09-25-migracao-vio-api-br-com-cache.md`, seção "Limpeza do resíduo `TESTE_STATUS_PROC` em `udlog_totem` (2026-09-26)": confirmado por leitura prévia que era 100% dado sintético (placa fake `EEE3333`, sem CPF/CNPJ/cliente/documento real associado). Transação única com chaves explícitas (`id_totem=2053`/`id_atendimento=9964`/`codigo='TESTE_STATUS_PROC'`, sem `LIKE`/curinga/`TRUNCATE`) removeu exatamente 3 registros: `tb_rate_limit_vio_status` (1, via `id_atendimento`), `tb_atendimento` (1), `tb_totem` (1) — `tb_atendimento_nota`/`tb_lgpd_aceite`/`tb_rate_limit_ocr` já estavam em 0 para este marcador. Contagens antes/depois conferidas ponto a ponto (`tb_totem` 9→8, `tb_atendimento` 127→126, `tb_rate_limit_vio_status` 5→4; `tb_atendimento_nota`=100 e `tb_rate_limit_ocr`=8 inalterados). Os outros 8 totens (`RECEPCAO-01` real + 7 sintéticos de outras demandas) confirmados intocados, mesmos `id_totem`. Fase 3 de testes pós-convergência (execução independente já registrada) resumida no mesmo handoff: 29 suítes, 1.350/1.350 asserções, 0 falhas. **Convergência do ambiente de desenvolvimento LOCAL considerada 100% concluída** (migrations 015/016 aplicadas + bateria de testes pós-convergência validada + resíduo sintético eliminado) — nenhuma pendência de limpeza de banco de dev remanescente para esta demanda. Nenhum commit/push feito nesta rodada. **`/03-revisao` INDEPENDENTE concluída em 2026-09-26 (veredito APROVADO COM RESSALVA)** — ver `docs/handoffs/2026-09-25-migracao-vio-api-br-com-cache.md`, seção "`/03-revisao` independente — veredito final (2026-09-26)": 4 revisores independentes novos (backend-especialista-revisor 10/10, security-especialista 6/7 confirmados + 1 confirmado parcialmente, frontend-especialista 7/7, qa-testes 5/5 com 38 suítes/1.492 asserções). 2 achados não-bloqueantes pendentes de correção antes do commit: (1) `DocumentoController::iniciarProcessamento()` tem bloco `try/finally` sem `catch(\Throwable)` ao redor de `enviarParaLeitura()`/`gravarIdExternoVioApiBr()`, diferente do padrão já usado em `statusProcessamento()` — depende de `display_errors=Off` em produção, ainda não documentado em `docs/deploy-checklist.md`; (2) comentários "ACHADO CRITICO" desatualizados em `tests/manual/qa_db_bootstrap.php`/`teste_vio_api_br_cas_e_cache.php` sobre suposto workaround da migration 015, contraditos por execução real. Zero chamada real, zero migration em banco real nesta revisão, zero commit/push. **Próximo passo: decisão do usuário entre uma rodada curta de correção destes 2 achados OU seguir direto para `/04-commit-e-push`.** **CORREÇÃO FINAL em 2026-09-26 (achado 1 resolvido)** — ver `docs/handoffs/2026-09-25-migracao-vio-api-br-com-cache.md`, seção "Correção final — catch ausente em `iniciarProcessamento()` (2026-09-26)": adicionado `catch(\Throwable $e)` ao bloco que envolve `enviarParaLeitura()`/`gravarIdExternoVioApiBr()` em `DocumentoController::iniciarProcessamento()`, mesmo padrão de log sanitizado (`logFalhaTecnica()`) já usado em `statusProcessamento()`; qualquer exceção não prevista (antes/durante/depois do envio) agora marca o documento como `INDETERMINADO` (nunca `ERRO`/`PENDENTE`) via `AtendimentoDao::marcarEnvioComoIndeterminado()` já existente, libera o lock (ordem `catch` antes de `finally` preservada) e devolve a mesma resposta HTTP 502 genérica já usada para ambiguidade, nunca expondo detalhe técnico. Validado com 3 exceções reais forçadas (antes/durante/depois do envio, via override de função por namespace para o transporte HTTP e via erro real de persistência), inclusive com `display_errors=1` forçado no próprio script de teste — resposta e log permaneceram sanitizados nos 3 casos, confirmando que a proteção é do código, não do ambiente; confirmado também que nenhum segundo POST ocorre em uma retentativa subsequente enquanto o estado é `INDETERMINADO`. `docs/deploy-checklist.md` ganhou item novo exigindo `display_errors=Off` em produção, explicitamente registrado como defesa COMPLEMENTAR (nunca dependência única). As 7 suítes desta demanda continuam 100% (77/77, 46/46, 29/29, 13/13, 43/43, 25/25, 29/29 — mesmas contagens já registradas), migrations 015/016 reconfirmadas idempotentes em banco `qa_` descartável. Achado (2) (comentários desatualizados em `tests/manual/qa_db_bootstrap.php`/`teste_vio_api_br_cas_e_cache.php`) **NÃO foi tocado nesta rodada** — fora do escopo desta correção pontual, permanece pendente. Zero chamada real/paga, zero credencial real, zero migration em banco real, zero commit/push nesta rodada. **CORREÇÃO FINAL em 2026-09-26 (achado 2 resolvido — limpeza de comentários pelo `qa-testes`)** — ver `docs/handoffs/2026-09-25-migracao-vio-api-br-com-cache.md`, seção "Rodada de limpeza de comentários (2026-09-26, qa-testes)": confirmação independente própria (3ª, além das 2 já feitas na `/03-revisao`) de que a migration 015 aplicada num banco `qa_` novo/descartável, sem chamar `qaDbCorrigirEnumsMigration015()`, produz corretamente os ENUMs esperados e é idempotente. Comentários "ACHADO CRITICO" reescritos em ambos os arquivos para refletir o estado atual (corrigido e validado), preservando nota histórica; `qaDbCorrigirEnumsMigration015()` virou no-op documentado (menor mudança de comportamento, ainda chamada por 4 outras suítes). Nenhuma lógica/asserção de teste alterada. As 7 suítes desta demanda seguem 100% (77/77, 46/46, 29/29, 13/13, 43/43, 25/25, 29/29). Zero commit/push. **Ambos os achados da `/03-revisao` estão agora resolvidos — demanda `migracao-vio-api-br-com-cache` sem pendências bloqueantes conhecidas, pronta para `/04-commit-e-push`.** |
