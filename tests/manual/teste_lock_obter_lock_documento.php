 ...
 *      NUNCA chama a vio.api.br real (VIO_API_BR_BASE_URL/API_KEY removidos
 *      do ambiente do subprocesso antes da chamada).
 ...
 *      TECNICA principal ja capturada antes do finally (VioApiBrClient()
...
// batia em "Column not found". Passa a rodar contra um banco `qa_`-prefixado
// DESCARTAVEL PROPRIO (mesmo helper tests/manual/qa_db_bootstrap.php ja
// usado pelas 3 suites novas), criado do zero e dropado ao final. Os
// subprocessos gerados abaixo (proc_open) recebem DB_NAME/demais envs
// herdados via putenv() do processo pai atraves de
// qaDbTrechoPonteEnvSubprocesso() (ver qa_db_bootstrap.php).
$nomeBancoLock0920 = null;
[$pdo, $nomeBancoLock0920] = qaDbCriar('lock_obter_lock_documento');
qaDbCorrigirEnumsMigration015($pdo);
// $_ENV (para as demais conexoes PDO abertas diretamente NESTE processo --
// novaConexaoIsolada()/novaConexaoLockEspiao() -- que leem $_ENV['DB_NAME']
// sem passar por subprocesso) + putenv() (para os subprocessos via
// proc_open(), que herdam via qaDbTrechoPonteEnvSubprocesso()).
$_ENV['DB_NAME'] = $nomeBancoLock0920;
putenv('DB_NAME=' . $nomeBancoLock0920);
register_shutdown_function(function () use (&$nomeBancoLock0920) {
    if ($nomeBancoLock0920 !== null) {
        qaDbDropar($nomeBancoLock0920);
        echo "\n(banco de teste {$nomeBancoLock0920} dropado)\n";
        $nomeBancoLock0920 = null;
    }
});
...
// (2026-09-26): forca App\Rn\VioApiBrClient (NUNCA mais
// App\Rn\VioDecodeClient) a falhar no construtor -- a falha tecnica
// PRINCIPAL sob teste aqui e exclusivamente a do LOCK, o cliente de
// validacao so precisa de UMA falha qualquer, sem ambiguidade, apos o lock
// ja ter sido tentado.
...
// Garante ZERO chamada de rede real a vio.api.br neste teste -- a falha
// tecnica sob teste aqui e exclusivamente a do LOCK, nunca a do cliente de
// validacao (que so precisa falhar de forma controlada, no construtor).
unset($_ENV['VIO_API_BR_BASE_URL'], $_ENV['VIO_API_BR_API_KEY']);
putenv('VIO_API_BR_BASE_URL');
putenv('VIO_API_BR_API_KEY');
...
// Reescrito na rodada corretiva de migracao-vio-api-br-com-cache
// (2026-09-26): falha PRINCIPAL agora via App\Rn\VioApiBrClient (NUNCA mais
// App\Rn\VioDecodeClient).
...
// rede real -- VioApiBrClient() falha no proprio construtor.
unset($_ENV['VIO_API_BR_BASE_URL'], $_ENV['VIO_API_BR_API_KEY']);
putenv('VIO_API_BR_BASE_URL');
putenv('VIO_API_BR_API_KEY');
...
// controlado (VIO_API_BR_BASE_URL/API_KEY ausentes) logo apos o trecho do
// lock, sem chamada de rede -- so para observar o comportamento do lock em
...
afirmar(
    ...
    str_contains($resItem2['log'], "iniciar-processamento: lock adicional nao adquirido (defesa em profundidade, CAS ja garantiu exclusividade) id_atendimento={$idAt2} tipo=crlv")
);
...
// script gerado remove VIO_API_BR_BASE_URL/API_KEY do ambiente do
// subprocesso).
// ============================================================
...
afirmar('Item 4: log sanitizado da falha de AQUISICAO do lock foi emitido (contexto fixo + get_class)', str_contains($resItem4['log'], "iniciar-processamento (obter lock adicional) id_atendimento={$idAt4} tipo=crlv: falha nao prevista [PDOException]"));
...
afirmar('Item 4: nenhuma chamada real a vio.api.br (log NAO contem nada do VioApiBrClient alem da falha local de construtor)', !str_contains($resItem4['log'], 'VioApiBrClient: falha tecnica categoria='));
...
// finally (VioApiBrClient() falhando no construtor, sem rede real).
...
afirmar('Item 6: log sanitizado da falha de LIBERACAO do lock foi emitido (contexto fixo + get_class, nunca getMessage/trace/SQL)', str_contains($resItem5['log'], "iniciar-processamento (liberar lock adicional) id_atendimento={$idAt5} tipo=crlv: falha nao prevista [PDOException]"));
afirmar('Item 8: resposta HTTP 503 da falha PRINCIPAL (VioApiBrClient) preservada, NAO mascarada pela falha de liberacao do lock', str_contains($resItem5['stdout'], 'HTTP_CODE:503'));
